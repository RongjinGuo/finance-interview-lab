...
test('hosted login, draft save, readable admin answers and mobile layout', async () => {
  ...
  const server=http.createServer(async(req,res)=>{
    ...
    if(url.pathname.startsWith('/api/')){
      ...
      if(url.pathname==='/api/login'){authenticated=true;data={user:users[0]};}
      ...
      else if(url.pathname==='/api/admin/users'&&req.method==='POST'){users.push({...body,role:'user'});data={user:users.at(-1)};}
  ...
  try{
    ...
    assert.equal(await page.locator('#account-username').isEnabled(),false,'username stays disabled until the session probe finishes');
    assert.equal(await page.locator('#account-password').isEnabled(),false,'password stays disabled until the session probe finishes');
    await page.locator('#account-username').fill('admin');await page.locator('#account-password').fill('browser-test-password-123');await page.locator('#account-login-submit').click();
    ...
    await page.locator('.account-create summary').click();await page.locator('#new-username').fill('charlie');await page.locator('#new-display-name').fill('新同学');await page.locator('#new-password').fill('browser-test-password-123');await page.locator('#account-create-user button').click();
